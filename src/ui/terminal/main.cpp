...
int main(int argc, char *argv[])
{
    ...
    if (cred["error"] == "true")
    {
        std::cerr << "Error when authenticating" << std::endl;
        return 1;
    }

   

    const std::string ip_mother_peer = "13.58.174.105";
...
// udp.udp_server();
    ...
    std::cout << "upnp_peer: " << ip_upnp_peer << ", my_ip: " << my_public_ip << std::endl;
    if (ip_upnp_peer == my_public_ip)
    {
        Upnp upnp;
        Udp udp;
    ...
    if (cred["new_peer"] == "true")
    {
        ...
        // if(upnp.Upnp_main() == 0) // upnp possible
        // {
            ip_next_peer = udp.udp_client(ip_upnp_peer, "None", new_peer);
