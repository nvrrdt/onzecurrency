...
- IMPORTANT idea to prevent a DOS attack: the block, including a nonce (!), is hashed and that hash leads to a disciple, which is again hashed (no nonce) with the disciple added leading to another disiple and that 10 (?) times, the tenth is the chosen_one and the chosen_one signs the whole block with a public key. The nonce should result in an amount of leading zero's in the first hash leading to a compute time of about 10 seconds on an average computer. It's not a race to find the first nonce because the chosen_one who gets some coins isn't known yet and it isn't possible to direct the chosen_one towards you. The 9 disciples and chosen_one exist to add some entropy in the chain. The hash, representing a user, above the calculated hash in the map is chosen to become disciple (= all nodes) or chosen_one (= fullnode), which also should add some entropy as users enter the map or leave. The 9 disciples and chosen_one will communicate each to ten remaining online users if all is ok with the block, otherwise the disiples and chosen_one will be informed and they verify and commmunicate accordingly. 10^10 = 10 billion, the exact n^n needed can be calculated.
